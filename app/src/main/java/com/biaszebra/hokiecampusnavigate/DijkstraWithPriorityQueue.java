  ...
  public static <N> List<N> findShortestPath(ValueGraph<N, Integer> graph, N source, N target) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (N neighbor : neighbors) {
        ...
        //replaced edgeValue with default, added default int 1, removed .orElseThrow(IllegalStateException::new) from being concatonated to parenthysese;
        int distance = graph.edgeValueOrDefault(node, neighbor,1);
